...
/// Stop-loss / take-profit configuration (added by local hard SL/TP feature).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct StopLossConfig {
    /// Enable or disable the local stop-loss / take-profit monitor.
    pub enabled: bool,
    /// Percentage below entry price to trigger stop-loss (0.15 = 15%).
    pub stop_loss_pct: Decimal,
    /// Percentage above entry price to trigger take-profit (0.30 = 30%).
    pub take_profit_pct: Decimal,
    /// How often (in seconds) the monitor checks all positions.
...
pub struct Config {
    ...
    // Stop-loss / take-profit (added by local hard SL/TP feature)
...
[stop_loss]
# Enable local hard stop-loss / take-profit monitor (true / false)
enabled = {sl_enabled}
# Percentage below entry price to trigger stop-loss (0.15 = 15%)
stop_loss_pct = {sl_pct}
# Percentage above entry price to trigger take-profit (0.30 = 30%)
take_profit_pct = {tp_pct}
# How often (seconds) the monitor checks all positions
