...
pub fn start_strategy_engine(
    ...
    freeze_guard: Arc<RwLock<crate::api_control::FreezeGuard>>,
    ...
    tokio::spawn(async move {
        ...
        // ── Wash-trade filter (added by wash_trade_filter feature) ──────────
        ...
        while let Some(event) = rx.recv().await {
            ...
            if eval.validated {
                ...
                // ── AI freeze guard (added by api_control feature) ─────────────
                if event.side == TradeSide::BUY {
                    let guard = freeze_guard.read().await;
                    if guard.is_frozen() {
                        warn!(
                            "BUY skipped: trading is FROZEN (AI freeze active) for token {}",
                            &event.token_id[..event.token_id.len().min(12)]
                        );
                        eval.validated = false;
                        eval.reason = Some("Trading frozen (AI freeze)".to_string());
                ...
                let order: Option<(OrderRequest, Decimal)> = if is_closing {
                    ...
                    // ── Slippage guard: reject if spread > 2.5% ──────────────────
                    ...
                    {
                        // ── Depth guard: reject if top-5 levels < 2000 USDC ──────
                        warn!(
                        ...
                        None
                    } else if wash_filter.is_wash_trade(&event.taker_address, &event) {
                        // ── Wash-trade guard: reject if same address same token ≥3 in 60s ──
                        warn!(
                        ...
                        if buy_size < MIN_ORDER_SHARES {
                            ...
                            buy_size, MIN_ORDER_SHARES, budget_usd, guard_limit_price
                        );
                            None
                        } else {
                            ...
                            let p = guard_limit_price;
                            let max_ctf_fee = buy_size
                                * Decimal::from_str("0.02").unwrap()
                                * p
                                * (Decimal::ONE - p);
                            let order_cost = buy_size * guard_limit_price;
                            ...
                            if current_balance < total_cost {
                                ...
                                if already_pending {
                                    ...
                                    // Use limit price from slippage guard (0.5% premium)
                                    Some((
                                        OrderRequest {
                                            token_id: event.token_id.clone(),
                                            price: guard_limit_price,
                                            size: buy_size,
                                            side: event.side,
                                        },
                                        total_cost,
                        ...
                        match submitter_clone(order).await {
                            Ok(()) => {
                                ...
                                if is_closing {
                                    ...
                                    // Record entry in stop-loss tracker
                                    {
                                        let mut sl = sl_state_clone.lock().await;
                                        sl.record_entry(token_id_clone.clone(), order_price);
                                    }
                                }
